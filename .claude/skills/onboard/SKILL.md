---
name: onboard
description: Interactive onboarding — sets up your first project, learns who you are, generates _core/identity.md profile, creates your first daily note, then loads vault context. For returning users, skips setup and loads context directly.
...
Set up your first project, run an interactive interview, generate a personalized `_core/identity.md`, create your first daily note, then load vault context. Returning users skip straight to context loading.
...
```
/onboard
  ├── _core/identity.md populated + config has real projects + no --reset?
  ...
  └── First time (no profile OR default projects OR --reset)?
       ...
       │   └── Create 01-index/{id}.md from template
       │
       ├── Phase 3: Personal Interview (3 rounds, 7 questions)
       │   └── Generate _core/identity.md
       │
...
Check if `_core/identity.md` has been populated (not just the default template):

```
Read: _core/identity.md
```
...
**Detection logic:**
- **Returning user**: `_core/identity.md` is populated (has a real role, not placeholder brackets) AND config does NOT contain `id: project-a` AND no `--reset` flag → Skip to Phase 5 (Load Context). Greet with:
...
5. **Create project index**: Write `01-index/{project-id}.md`:
...
Map answers to `_core/identity.md`. Write to the bootloader directory.
...
Write the file using `Write` tool to `_core/identity.md`:

```markdown
# Personal Profile

> Generated by `/onboard` on {today's date}. Edit anytime — tracked in your vault.
...
_core/identity.md      → Personal profile (just generated or existing)
```

### 3. Project Indexes
```
01-index/{project}.md  → MOC for each active project
```
...
> **You're all set, {role}!**
>
> Your profile is saved to `_core/identity.md`. Here's what I set up:
> - Project: **{project display name}** (`{project-id}/`)
> - Config: {n} active projects
> - Index: `01-index/{project-id}.md`
