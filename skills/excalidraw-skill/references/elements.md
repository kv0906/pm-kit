...
### Frame Ordering ⚠️ IMPORTANT

Frames have a **critical ordering requirement** in the elements array. Frame children must come **before** the frame element itself:

```json
[
  other_element,
  frame1_child1,      // ← Children first
  frame1_child2,      // ← Children first
  frame1,             // ← Frame last
  other_element,
  frame2_child1,      // ← Children first
  frame2_child2,      // ← Children first
  frame2,             // ← Frame last
  other_element
]
```
...
**Why this matters**:
- ✅ Correct order: Children render inside frame, clipping works properly
- ❌ Wrong order: Elements may not render or clip correctly
- ⚡ Performance: Renderer relies on this ordering for optimizations

**Rule**: Always add frame children to the elements array first, then add the frame element itself.
